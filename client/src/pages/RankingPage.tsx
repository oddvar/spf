...
export default function RankingPage() {
  ...
  const [oddvarFinalTeamCount, setOddvarFinalTeamCount] = useState(0);
  ...
  useEffect(() => {
    ...
    // Fetch oddvar's final match teams to calculate max final bonus points
    get<any>('/knockout/oddvar-final')
      .then((data) => {
        if (data.finalMatch) {
          const teams = new Set<string>();
          const finalMatch = Array.isArray(data.finalMatch) ? data.finalMatch[0] : data.finalMatch;
          if (finalMatch) {
            if (finalMatch.home_team && finalMatch.home_team !== '?') teams.add(finalMatch.home_team);
            if (finalMatch.away_team && finalMatch.away_team !== '?') teams.add(finalMatch.away_team);
          }
          setOddvarFinalTeamCount(teams.size);
        }
      })
      .catch(() => {
        // Silently fail
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (cutoffMatchNumber === '' || cutoffMatchNumber === 'all+r32' || cutoffMatchNumber === 'all+r32+r16' || cutoffMatchNumber === 'qf+r16+r32+group' || cutoffMatchNumber === 'sf+qf+r16+r32+group' || cutoffMatchNumber === 'f+sf+qf+r16+r32+group' || cutoffMatchNumber === 'tp+f+sf+qf+r16+r32+group' || cutoffMatchNumber === 'w+tp+f+sf+qf+r16+r32+group' || cutoffMatchNumber === 'all') {
  ...
  const getMaxPointsDisplay = () => {
    ...
    if (cutoffMatchNumber === 'all' && rankings.length > 0 && groupStageMaxPoints > 0) {
      // Display all column maxes: group, r32, r16, qf, sf, final, third_place_winner, winner
      ...
      const tpWinnerMax = rankings[0]?.maxPossibleScore > 0 ? (rankings.some(r => r.thirdPlaceScore > 0) ? 7 : 0) : 0;
      const winnerMax = rankings[0]?.maxPossibleScore > 0 ? (rankings.some(r => r.winnerScore > 0) ? 15 : 0) : 0;
      const total = groupStageMaxPoints + r32AdvancementMax + r16BonusMax + qfBonusMax + sfBonusMax + fBonusMax + tpWinnerMax + winnerMax;
      return `${groupStageMaxPoints}+${r32AdvancementMax}+${r16BonusMax}+${qfBonusMax}+${sfBonusMax}+${fBonusMax}+${tpWinnerMax}+${winnerMax}=${total}`;
    }
    ...
    if (cutoffMatchNumber === 'tp+f+sf+qf+r16+r32+group' && rankings.length > 0 && groupStageMaxPoints > 0) {
      // Display all column maxes: group, r32, r16, qf, sf, final, third_place_winner
      const r32AdvancementMax = oddvarR32TeamCount * 2;
      const r16BonusMax = oddvarR16TeamCount * 3;
      const qfBonusMax = oddvarQFTeamCount * 4;
      const sfBonusMax = oddvarSFTeamCount * 5;
      const fBonusMax = oddvarFinalTeamCount * 6;
      const tpWinnerMax = rankings.some(r => r.thirdPlaceScore > 0) ? 7 : 0;
      const total = groupStageMaxPoints + r32AdvancementMax + r16BonusMax + qfBonusMax + sfBonusMax + fBonusMax + tpWinnerMax;
      return `${groupStageMaxPoints}+${r32AdvancementMax}+${r16BonusMax}+${qfBonusMax}+${sfBonusMax}+${fBonusMax}+${tpWinnerMax}=${total}`;
    }
    if (cutoffMatchNumber === 'w+tp+f+sf+qf+r16+r32+group' && rankings.length > 0 && groupStageMaxPoints > 0) {
      // Display all column maxes: group, r32, r16, qf, sf, final, third_place_winner, winner
      const r32AdvancementMax = oddvarR32TeamCount * 2;
      const r16BonusMax = oddvarR16TeamCount * 3;
      const qfBonusMax = oddvarQFTeamCount * 4;
      ...
      const fBonusMax = oddvarFinalTeamCount * 6;
      const tpWinnerMax = rankings.some(r => r.thirdPlaceScore > 0) ? 7 : 0;
      const winnerMax = rankings.some(r => r.winnerScore > 0) ? 15 : 0;
      const total = groupStageMaxPoints + r32AdvancementMax + r16BonusMax + qfBonusMax + sfBonusMax + fBonusMax + tpWinnerMax + winnerMax;
      return `${groupStageMaxPoints}+${r32AdvancementMax}+${r16BonusMax}+${qfBonusMax}+${sfBonusMax}+${fBonusMax}+${tpWinnerMax}+${winnerMax}=${total}`;
    }
      ...
      <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            else if (val === 'w+tp+f+sf+qf+r16+r32+group') setCutoffMatchNumber('w+tp+f+sf+qf+r16+r32+group');
        ...
        >
          ...
          <option value="w+tp+f+sf+qf+r16+r32+group">group, r32, r16, qf, sf, final, 3rd and winner</option>
              ...
              {rankings.map((user) => {
                ...
                return (
                  <tr key={user.user_id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '0.5rem', textAlign: 'right' }}>
                      {(cutoffMatchNumber === 'all' || cutoffMatchNumber === 'tp+f+sf+qf+r16+r32+group' || cutoffMatchNumber === 'w+tp+f+sf+qf+r16+r32+group') ? user.thirdPlaceScore : ''}
                    </td>
                    <td style={{ padding: '0.5rem', textAlign: 'right' }}>
                      {(cutoffMatchNumber === 'all' || cutoffMatchNumber === 'w+tp+f+sf+qf+r16+r32+group') ? user.winnerScore : ''}
                    </td>
